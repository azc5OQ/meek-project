...
if ! command -v pkg-config >/dev/null; then
    ...
    for pkg in wayland-client wayland-egl wayland-protocols egl glesv2; do
...
WL_CFLAGS="$(pkg-config --cflags wayland-client wayland-egl egl glesv2)"
WL_LIBS="$(pkg-config --libs   wayland-client wayland-egl egl glesv2)"
...
stage_assets() {
    ...
    mkdir -p "$dst" "$dst/fonts" "$dst/wallpapers"
