//
// demo-linux-x11/main.c - Linux X11 host for the gui library.
//
// Desktop-Linux counterpart to demo-linux-drm. Runs as a normal X11
// client -- no DRM master, no tty switch, no "stop the display
// manager" dance. Coexists with Xorg / Xfce / KDE / GNOME / anything
// running a compositor; shows up as a regular application window.
//
// SHARES the declarative parts (main.ui + main.style) with the
// Windows, Android, and DRM demos -- build.sh stages them next to
// the binary.
//
// Same UI_HANDLER set as demo-linux-drm -- the library auto-resolves
// them via dlsym(RTLD_DEFAULT, name), and the handler bodies differ
// only in the log prefix.
//
...
UI_HANDLER void on_primary_click(gui_event* ev)      { (void)ev; log_info("[linux-x11] primary click"); }
UI_HANDLER void on_primary_click1(gui_event* ev)     { (void)ev; log_info("[linux-x11] primary click (second button)"); }
UI_HANDLER void on_secondary_click(gui_event* ev)    { (void)ev; log_info("[linux-x11] secondary click"); }
...
UI_HANDLER void on_theme_toggle(gui_event* ev)
{
    ...
    log_info("[linux-x11] theme: %s", g_dark_theme ? "dark" : "light");
}
...
UI_HANDLER void on_popup_dismissed(gui_event* ev)      { log_info("[linux-x11] popup dismissed: %s", ev->popup.confirmed ? "yes/ok" : "no/cancel"); }
