//
// demo-macos/main.c - macOS host for the gui library.
//
// Cocoa + OpenGL 4.1 Core via platform_macos.m, with the same
// UI_HANDLER set the Windows / Android / Linux DRM / Linux X11
// demos ship. The library auto-resolves handlers via dlsym
// (RTLD_DEFAULT, name), same pattern every POSIX target uses.
//
// SHARES main.ui + main.style with ../demo-windows/; build.sh
// stages them next to the binary.
//
// The host .c file is pure C (not Objective-C). Only the platform
// backend needs .m -- which is why this file is main.c, not
// main.m. The compiler / build script treat it as a plain C TU.
//
...
UI_HANDLER void on_primary_click(gui_event* ev)      { (void)ev; log_info("[macos] primary click"); }
UI_HANDLER void on_primary_click1(gui_event* ev)     { (void)ev; log_info("[macos] primary click (second button)"); }
UI_HANDLER void on_secondary_click(gui_event* ev)    { (void)ev; log_info("[macos] secondary click"); }
...
UI_HANDLER void on_theme_toggle(gui_event* ev)
{
    ...
    log_info("[macos] theme: %s", g_dark_theme ? "dark" : "light");
}
...
UI_HANDLER void on_popup_dismissed(gui_event* ev)      { log_info("[macos] popup dismissed: %s", ev->popup.confirmed ? "yes/ok" : "no/cancel"); }
