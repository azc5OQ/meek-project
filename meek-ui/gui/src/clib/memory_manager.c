...
//
    ...
    static void _memory_manager_internal__mutex_init   (_memory_manager_internal__mutex* m) { InitializeCriticalSection(m); }
    static void _memory_manager_internal__mutex_destroy(_memory_manager_internal__mutex* m) { DeleteCriticalSection(m); }
    static void _memory_manager_internal__mutex_lock   (_memory_manager_internal__mutex* m) { EnterCriticalSection(m); }
    static void _memory_manager_internal__mutex_unlock (_memory_manager_internal__mutex* m) { LeaveCriticalSection(m); }
#else
    #include <pthread.h>
    typedef pthread_mutex_t _memory_manager_internal__mutex;
    static void _memory_manager_internal__mutex_init   (_memory_manager_internal__mutex* m) { pthread_mutex_init(m, NULL); }
    static void _memory_manager_internal__mutex_destroy(_memory_manager_internal__mutex* m) { pthread_mutex_destroy(m); }
    static void _memory_manager_internal__mutex_lock   (_memory_manager_internal__mutex* m) { pthread_mutex_lock(m); }
    static void _memory_manager_internal__mutex_unlock (_memory_manager_internal__mutex* m) { pthread_mutex_unlock(m); }
#endif
...
void* memory_manager__realloc(void* ptr, size_t new_size)
{
    ...
    {
        _memory_manager_internal__mutex_unlock(&_memory_manager_internal__lock);
        log_error("memory_manager__realloc: bad guard on %p (not a tracked block, or corrupted)", ptr);
        return NULL;
    }
    ...
    if (new_h == NULL)
    {
        ...
        old_h->prev = prev;
        old_h->next = next;
        if (prev != NULL) { prev->next = old_h; } else { _memory_manager_internal__head = old_h; }
        if (next != NULL) { next->prev = old_h; }
        _memory_manager_internal__mutex_unlock(&_memory_manager_internal__lock);
        return NULL;
    }
    ...
    if (prev != NULL) { prev->next = new_h; } else { _memory_manager_internal__head = new_h; }
    if (next != NULL) { next->prev = new_h; }
...
void memory_manager__free(void* ptr)
{
    ...
    {
        _memory_manager_internal__mutex_unlock(&_memory_manager_internal__lock);
        log_error("memory_manager__free: bad guard on %p (not a tracked block, or corrupted header)", ptr);
        return;
    }
