...
//char*				clib__int_to_hex_string(int n);
//char*				clib__int_to_string(int i, char* b);
...
const char *clib__strstr(const char *s1, const char *s2); //zisti ci sa v stringu nachazda iny string
int clib__strcmp(const char *str1, const char *str2); //navrati 0 ak sa stringy rovnaju.
int clib__strncmp(const char *str1, const char *str2, int number_of_bytes_to_compare); //navrati 0 ak sa stringy rovnaju.
int clib__stricmp(const char *str1, const char *str2); //navrati 0 ak sa stringy rovnaju, avsak nezalezi na tom ci su velke alebo male pismenko
...
boole clib__is_bit_active_in_uint32(uint value, int bit_index); //bit index of 0, LSB bit
...
void *__cdecl memmove(void *destination, void const *source, size_t length); ///nazov tejto metody zda sa nemoze byt iny..
#endif
...
float clib__floor32(float x); //tato funkcia nieje o zaokruhlovani, aj ked maze sa pri jej pouzity z hodnoty desatinna ciarka. floor prijme realne cislo, a navrati prve mensie cislo prirozdene,
...
double clib__ln(const double n); //na kolko sa musi umocnit eulerovo cislo aby som dostal n?
double clib__log(const double n, const double base); //na kolko sa musi umocnit base aby som dostal n?
uint clib__modulo(uint divident, uint divisor);
void clib__zerostring(char *Str);

//bool_				clib__create_file_from_buffer(char* new_file_name, byte* buffer, size_t length);

//void				clib__qsort(); toto som nevedel implementovat, na neskor. zatial pouzivam qsort s mvscrt, aj ked msvcrt dost pada
