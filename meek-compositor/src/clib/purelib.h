 ...
 *
 * @param const char *str -> a pointer to the null-terminated string to be converted
 * @param char **endptr
 * @param int base -> a base of a number to be converted (function needs this to tell if string number is hexadecimal or decimal)
 *
 *
 * @return long
 */
...
/**
 * @brief compares two strings, returns 0 (FALSE) if they are EQUAL, difference between strcmp is that this also specifies max number of chars to compare
 *
 * @param const char *str1
 * @param const char *str2
 * @param size_t n
 ...
 * @return int
 */
int purelib__strncmp(const char *str1, const char *str2, size_t n); //compares specific number of characters, //returns 0 if strings are equal

/**
 * @brief compares two strings, returns 0 (FALSE) if they are EQUAL, ignores the case (doesnt matter if big or small letters)
 *
 * @param const char *str1
 * @param const char *str2
 *
