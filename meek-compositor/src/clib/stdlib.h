...
/** ASCII lowercase conversion; non-letters pass through. @function stdlib__tolower */
int stdlib__tolower(int c);

/** ASCII uppercase conversion; non-letters pass through. @function stdlib__toupper */
 ...
 * @param {const char*} s
 * @param {char**} endptr - receives pointer past the last parsed char (may be NULL).
...
// libc function.
//
static inline float stdlib__sqrtf (float x)           { return __builtin_sqrtf(x); }
static inline float stdlib__fabsf (float x)           { return __builtin_fabsf(x); }
static inline float stdlib__floorf(float x)           { return __builtin_floorf(x); }
static inline float stdlib__ceilf (float x)           { return __builtin_ceilf(x); }
static inline float stdlib__fmodf (float x, float y)  { return __builtin_fmodf(x, y); }
static inline float stdlib__fminf (float x, float y)  { return (x < y) ? x : y; }
static inline float stdlib__fmaxf (float x, float y)  { return (x > y) ? x : y; }
//
// Double variants — rare in the toolkit but a few colour-space +
// font paths use them. Kept for drop-in compat with existing code.
//
static inline double stdlib__sqrt (double x)          { return __builtin_sqrt (x); }
static inline double stdlib__fabs (double x)          { return __builtin_fabs (x); }
static inline double stdlib__floor(double x)          { return __builtin_floor(x); }
static inline double stdlib__ceil (double x)          { return __builtin_ceil (x); }
static inline double stdlib__fmod (double x, double y){ return __builtin_fmod (x, y); }
#else
