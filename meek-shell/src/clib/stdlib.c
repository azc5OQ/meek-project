...
int stdlib__atoi(const char *s)
{
	...
	//
	while (stdlib__isspace((unsigned char)*s))
	{
		s++;
	}
	int sign = 1;
...
long stdlib__strtol(const char *s, char **endptr, int base)
{
	...
	const char *p = s;
	while (stdlib__isspace((unsigned char)*p))
	{
		p++;
	}
	int neg = 0;
...
float stdlib__powf(float a, float b)
{
	return powf(a, b);
}
