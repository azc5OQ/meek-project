...
// to fix
// double and float functions mix up, separate functions that deal with doubles
// and floats it does not crash, but its probably also not right way to do
// things
...
char **clib__split_str(char *string_to_split, int string_to_split_length, char *substring_used_in_splitting, int substring_used_in_splitting_length, int *splitted_substrings_count)
{
	...
	// in function arguments, change the value of "array_of_substrings" pointer so
	// it will point to pointer that holds all found substrings

	// one - dymensional array where substring positions inside string will be
	// stored dynamically allocated to appropiate size

	// if found substring will be 3 chars long and located at the begging on base
	// string its positions will be 0,2 because its allocated with malloc, array
	...
	for (x = 0; x < string_to_split_length; x++)
	{
		...
		// if chars do not match, skip check for current base position and continue
		// with next one
		int i;
		for (i = 0; i < substring_used_in_splitting_length; i++)
		{
			...
			// if char matches but it matches with a ending character that cant be
			// used to split any string (there are no chars behind it) break
			...
			{
				// if searched string is found at beginning, what will change is
				// found_substring start position
				if (x == 0)
				{
				...
				{
					// store beginning and end positions of found substring in dynamically
					// allocated array
	...
	// find what should be the length of final array where pointers will be
	// stored. the same loop will be repeased later but for the purpose of
	// extracting individual chars from base string and putting them to substrings
	// which will be then put to newly allocated array

	int actual_number_of_strings = 0;

	int y;
	// iteration over found positions to count strings whose length is greater
	// than 0
	...
	if (actual_number_of_strings > 0)
	{
		...
		// from positions of found target strings, decude positions of strings that
		// will be part of resu�ting array of substrings
...
float clib__atof(char *pstr)
{
	...
	{
		// za alebo pred desatinnou ciarkou sa nenachazda cislica. napriklad .124
		// alebo 124. alebo .
		return 0;
	}
...
void clib__str_to_float(float f, char *str, ubyte precision)
{
	...
	/*
extracting most significant digit i.e. right most digit , and concatenating to
string obtained as quotient by dividing number by 10^k where k = (number of
digit -1)
*/
...
static double bp[] = {
	...
	zero = 0.0, two = 2.0, two53 = 9007199254740992.0, /* 0x43400000, 0x00000000 */
	huge = 1.0e300,
...
std::cout << "x: " << first_string_current_index << " : " <<
(uint)first[first_string_current_index] << " : " <<
(uint)first[first_string_current_index + 1];*/

		// unicode string is, or should, be terminated by two zeroes.
		// to find out where unicode string ends, string address space must be
		// scanned from the start of the string the code must check if one of the
		// null bytes does not belong to some other character. Because unicode chars
		// also contain null bytes only if two null bytes both belong to same
		// character, can end of the string considered to be found

		// better option that to check whether two null bytes belong to same wide
		// char would be to only check bytes from same wide char
...
std::cout << "first_string_length hex " << std::hex << first_string_length <<
std::endl;*/
	...
	for (;;)
	{
		if (second[second_string_current_index] == 0 && second[second_string_current_index + 1] == 0)
		{
			...
			/*       std::cout << "second_string_length " << second_string_length <<
 std::endl; std::cout << "second_string_length hex " << std::hex <<
 second_string_length << std::endl;*/
	...
	// it is assumed that there is aditional space available in the buffer of
	// first string place in RAM where unicode string ends is null terminated by
	// two strings not one but that is wrong to assume. you dumb dumb null out
	// aditional two bytes in wide string, so resulting string is null terminated
...
// #pragma optimize("", off) aby visual studio nenahrazdoval moju memset funkciu
// standartnou memset funkciou
...
// assembly som debugoval pred pouzitim, funguje, nevyhoda je ze prijma len 32
// bitove hodnoty, ale postaci
...
{
	// isnt there like a.. assemblt instruction for this? that would make it a lot
	// faster like real strcmp, we return FALSE if strings are EQUAL true if not
	// equal
	boole result = TRUE;
...
{
	// ci je adresa platna na citanie  skontrolovat inde cez prislusnu api
	// prieliehajucu operacnemu systemu ASCII string
...
nuint clib__find_address_in_module_address_space_use_pattern(HMODULE moduleHandle, cstring pattern, nuint offset)
{
	...
	memset((void *)&moduleInfo, 0,
		sizeof(MODULEINFO)); // bezpecnost pri praci s pamatou
...
nuint clib__find_address_in_module_address_space_use_pattern_my_version(HMODULE moduleHandle, cstring pattern, nuint offset)
{
	...
	// console_write("%s %d %s",
	// "|clib__search_for_pattern_in_module_address_space| number_of_bytes_to_scan
	// ", number_of_bytes_to_scan, "\n");

	nuint result = 0;
	int whitespace_count = 0;
	size_t i;

	// ziska sa pocet mezdier v skanovanom stringu. Tak sa zisti kolko bytov sa
	// hlada
	...
	for (i = 0; i < size_of_address_space_to_search_in; i++)
	{
		for (ii = 0; ii < buffer_length; ii++)
		{
			...
			if (read_byte != bytes_to_find[ii])
			{
				...
				// console_write("%s %d %s %d %s", "read_byte ", read_byte, "!=",
				// bytes_to_find[ii],"\n");
#endif
			...
			else if (read_byte == bytes_to_find[ii])
			{
				...
				// printf("%s %d %s %d %s", "read_byte ", read_byte, " EQUALS ",
				// bytes_to_find[ii], "\n");
#endif
			}
...
// bool_ clib__create_file_from_buffer(char* new_file_name, byte* _buffer,
// size_t _length)
//{
//     //this is wrong
//     FILE* pFile;
...
//     {
// #ifdef USING_NO_DEFAULT_LIB
//         console_write("%s%s%s", "[!] fopen(wb) on file path ", new_file_name,
//         " failed \n");
// #endif
//         return FALSE;
//     }
...
//
// #ifdef USING_NO_DEFAULT_LIB
//     console_write("%s%s%s", "clib__create_file_from_buffer ", new_file_name,
//     " success \n");
// #endif
//
//     return TRUE;
...
uint64 clib__hexstring_to_uint64(char *hex_string)
{
	...
	// console_write("%s %d %s", "clib__hexstring_to_uint64 input length ",
	// string_length1, "\n"); console_write("%s %d %s", "clib__hexstring_to_uint64
	// input length ", string_length2, "\n");
	...
	for (i = 0; hex_string[i] != '\0'; i++)
	{
		...
		parsed_number = parsed_number * 16 + (nuint)value; // multiply by 16 for correct digit shifting
		// because we're iterating left to right.
	}
...
int clib__strcmp(const char *str1, const char *str2)
{
	...
	return (int)*str1 - (int)*str2; // ak jeden string narazil na null terminator,
	// mal by narazit aj druhy.
}
...
{
	int current_byte = 0;
	while (current_byte < number_of_bytes_to_compare) // pokial obidva stringy nenarazia na null
	// terminator
	{
		if (*str1 != *str2)
		{
...
// navrati 0 ak sa stringy rovnaju, avsak nezalezi na tom ci su velke alebo male
// pismenko
...
// od delenca (x),  x, sa odcita cast ktora sa da rozdelit
// (float)clib__truncate(x / y) * y;, to je zvysok
...
// void clib__qsort(void* v, int size, int left, int right, int (*comp)(void*,
// void*))
//{
//     void* vt, * v3;
