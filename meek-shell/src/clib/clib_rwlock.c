 ...
 * KEY INVARIANTS
 * --------------
 * 1. num_readers == 0 whenever write_recursion > 0 (writer has exclusive
 * access).
 ...
 * the lock at the time of this call. Drains any leftover queue nodes
 * defensively (there should be none in a correct shutdown) to avoid leaking
 * condvars.
 */
...
/* -----------------------------------------------------------------------------
   ...
   Acquisition order:
     Fast-path 1  — this thread already holds the WRITE lock (write→read
   nesting) Fast-path 2  — this thread already holds a READ lock (recursive
   read) Slow path    — first acquisition: take FIFO ticket and wait
   -----------------------------------------------------------------------------
 */
...
/* -----------------------------------------------------------------------------
   ...
   Identifies which lock type this thread last acquired and releases one level:
     ...
     A. Writer with nested reads outstanding (write_recursion > 0 AND
   my_read_count > 0) → release one nested read (TLS counter only; num_readers
   was never incremented for writer-nested reads per fast-path 1 above).
   ...
   matches each clib__read_lock / clib__write_lock with exactly one
   clib__unlock, the nested reads are always released before the write lock.
   -----------------------------------------------------------------------------
 */
